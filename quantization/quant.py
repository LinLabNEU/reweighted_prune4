...
#B = 4
    ...
    for n in range(0, N):
        ...
        r  -= q
    print(qIdx)
    return torch.FloatTensor(wT*wMax)
...
def test():
    ...
    bits=8
    overflow_rate=0.1
    compute_integral_part(input,overflow_rate)
...
def log_minmax_quantize(input, bits):
    ...
    v = torch.exp(v) * s
    return v
def log2_minmax_quantize(input, bits):
    input_l=input.tolist()
    assert bits >= 1, bits
    if bits == 1:
        return torsor.FloatTensor(np.sign(input_l)), 0.0, 0.0
    s = np.sign(input_l)
    input0 = np.log2(np.abs(input_l) + 1e-20)
    input1 = np.floor(input0)
    v = min_max_quantize(torch.FloatTensor(input0), bits)
    print("V",input1)
    #v = np.power(2,v.tolist()) * s
    v = np.power(2,input1) * s
